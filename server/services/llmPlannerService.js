...
function getCatalogReply(catalog, message) {
  ...
  if (!catalog.length) return { reply: 'The catalog is empty right now.', productId: null };
  if (!selectedProduct) {
    return {
...
export async function createCatalogChatReply({ catalog, message, history = [] }) {
  ...
  try {
    ...
    return product ? { reply: result.reply, productId: product.id } : getCatalogReply(catalog, message);
  } catch {
